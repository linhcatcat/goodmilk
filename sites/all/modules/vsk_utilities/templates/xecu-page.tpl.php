...
<div class="wrap-home-page wrap-product">
	<ul>
		...
		<li data="row-<?php print $i; ?>" class="row-item"><?php print l($obj_rs->node_title,'xe-cu/'.$obj_rs->nid.'/'.unicode_str_replace($obj_rs->node_title)) ?>
		...
		$obj1 = db_query("SELECT node.nid AS nid,
			...
			WHERE (node.status <> 0) AND (node.type in ('xecu')) AND (node_data_field_xecu_loaixe.field_xecu_loaixe_nid = '". $obj_rs->nid ."') 
			ORDER BY node_created ASC
			");
				?>
				<li>
				<?php 
					...
					$path = $arr['filepath'];
					$url = $base_url .'/chi-tiet-xc/'. $obj_rs1->nid .'/gioi-thieu';
					if($i%4==0){
						$cls = " first";
					} else {
						$cls = "";
				...
				?>
					<div class="item<?php print $cls; ?>">
						...
						<div class="title"><?php print l($obj_rs1->node_title, '/chi-tiet-xc/'. $obj_rs1->nid .'/gioi-thieu'); ?></div>
