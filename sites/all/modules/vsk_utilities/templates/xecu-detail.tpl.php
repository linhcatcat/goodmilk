...
<div class="product-detail">
	...
	<div class="detail-01">
		<div class="left"><img src="<?php print $base_url .'/'.$node->field_xecu_hinhanh[0]['filepath']; ?>" /></div>
		<div class="right">
			<div class="title"><?php print $node->title; ?></div>
			<div class="summ"><?php print $node->field_xecu_tomtat[0]['value']; ?></div>
		</div>
		<div class="clear"></div>
		<div class="intro"><?php print $node->field_xecu_gioithieu[0]['value']; ?></div>
	</div>
	<?php } elseif(arg(2)=='thong-so-ki-thuat'){ ?>
		...
		<div class="detail-02">
			...
			<div class="intro"><?php print $node->field_xecu_thongsokythuat[0]['value']; ?></div>
		</div>
		<?php } else{ ?>
	...
	<?php }elseif(arg(2)=='trang-bi-tieu-chuan'){ ?>
		...
		<div class="detail-02">
			...
			<div class="intro"><?php print $node->field_xecu_trangbitieuchuan[0]['value']; ?></div>
		</div>
		<?php } else{ ?>
			<div class="center">Đang cập nhật thông tin!</div>
		<?php } ?>
	<?php }elseif(arg(2)=='thu-vien-anh'){ ?>
		<?php if($node->field_xecu_thuvienanh){ ?>
		<div class="detail-03">
			<div class="title"><?php print 'Thư viện ảnh'; ?></div>
			<div id="thu-vien-anh">
				<div class="left">
					<ul>
						<?php foreach($node->field_xecu_thuvienanh AS $key => $value){ ?>
				...
				<div class="right">
					<?php foreach($node->field_xecu_thuvienanh AS $key1 => $value1){ ?>
	...
	<?php }elseif(arg(2)=='tinh-nang-lai-uu-viet'){ ?>
		<?php
			$aProducts = get_xecu($pid, 7);
		?>
		<?php if(count_xecu($pid, 7)){ ?>
				...
				<div class="left">
					<ul>
						...
						foreach($aProducts as $obj_rs2){
							$arr = field_file_load($obj_rs2->thongsoxecu_hinhanh_fid);
							$path = $arr['filepath'];
						?>
				...
				<div class="right">
					<?php 
						...
						foreach($aProducts as $obj_rs3){
							$arr = field_file_load($obj_rs3->thongsoxecu_hinhanh_fid);
							$path = $arr['filepath'];
						?>
	...
	<?php }elseif(arg(2)=='thiet-ke-hien-dai'){ ?>
		<?php
			$aProducts = get_xecu($pid, 6);
		?>
		<?php if(count_xecu($pid, 6)){ ?>
		<div class="detail-03 detail-04">
	...
	<?php }elseif(arg(2)=='thiet-ke-linh-hoat'){ ?>
		<?php
			$aProducts = get_xecu($pid, 5);
		?>
		<?php if(count_xecu($pid, 5)){ ?>
		<div class="detail-03 detail-04">
	...
	<?php }elseif(arg(2)=='ngoai-that'){ ?>
		<?php
			$aProducts = get_xecu($pid, 4);
		?>
		<?php if(count_xecu($pid, 4)){ ?>
		<div class="detail-03 detail-04">
	...
	<?php }elseif(arg(2)=='noi-that'){ ?>
		<?php
			$aProducts = get_xecu($pid, 3);
		?>
		<?php if(count_xecu($pid, 3)){ ?>
		<div class="detail-03 detail-04">
	...
	<?php }elseif(arg(2)=='cac-diem-noi-bat'){ ?>
		<?php
			$aProducts = get_xecu($pid, 2);
		?>
		<?php if(count_xecu($pid, 2)){ ?>
		<div class="detail-03 detail-04">
	...
	<?php }elseif(arg(2)=='dac-tinh-cong-nghe'){ ?>
		<?php
			$aProducts = get_xecu($pid, 1);
			//smo($aProducts);
		?>
		<?php if(count_xecu($pid, 1)){ ?>
		<div class="detail-03 detail-04">
